...
> **[Document Metadata]**
> - **상태(Status)**: 완료됨/보관됨 (Completed / Archived)
> - **보관 일자(Date)**: 2026-05-26
> - **보관 사유(Reason)**: 초기 마스터 구현 계획에 따른 주요 마일스톤(Phase 1~4)이 사실상 일단락되었으며, 이후 세부 태스크는 별도 트래킹이나 신규 계획서로 관리하기 위해 본 문서를 완료(스냅샷) 처리함.
...
## Phase 5: 오프라인 퍼스트 및 로컬 데이터 동기화 (Outbox Pattern) [향후 과제]

현재 100% 서버 통신에 의존하고 있는 클라이언트 앱(`edu-client`)의 오답 기록 아키텍처를 개편하여, 네트워크가 없는 오프라인 환경에서도 학습 데이터가 유실되지 않도록 보장합니다.

### 1. 클라이언트 (Flutter) 변경 사항
*   **로컬 DB (SQLite / Hive) 연동**: `sqflite` 또는 `hive`를 도입하여 오프라인 시 오답 데이터를 저장할 `client_incorrect_note` 로컬 테이블 구성.
*   **Outbox Pattern (`isSynced` 플래그) 적용**: 인터넷이 단절되었을 때 생성되거나 삭제된 오답 항목에 대해 로컬 DB에 `isSynced = false` 로 마킹하여 큐(Queue) 형태로 보관.
*   **동기화 백그라운드 워커**: `connectivity_plus` 등을 활용해 인터넷 연결 복구를 감지하면, 서버의 벌크 동기화 API로 로컬의 미동기화(`isSynced = false`) 데이터를 일괄 Push하고 로컬 상태 업데이트.

### 2. 백엔드 (Spring Boot) 변경 사항
*   `POST /api/sync/incorrect-notes`: 클라이언트에서 Push하는 복수의 오답 기록 데이터를 멱등성(Idempotent) 있게 Upsert 처리하는 벌크 동기화 수신 엔드포인트 신설.
