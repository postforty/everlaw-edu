...
2. **Python 기반 LLM 서버 (AI Engine)**: **FastAPI**와 **LangChain**을 활용하여 차분 분석, RAG 기반 콘텐츠 생성 및 교차 검증 수행. (Spring Boot와 gRPC/REST API 통신)
3. **Redis (Cache & Message Broker)**: 비동기 작업 큐(Task Queue) 처리, LLM 응답 데이터 캐싱 및 실시간 알림 상태 관리.
4. **Flutter (Front-end)**: 관리자용 웹 대시보드 및 학습자용 멀티 플랫폼(Android, iOS, Web) 클라이언트 제공.
...
- [x] **T1.2. AI Engine (Python/FastAPI) 환경 및 스켈레톤**
    ...
    - [x] `.env` 설정 및 인프라 연결 정보 구성
- [x] **T1.3. RAG 파이프라인 PoC 구현**
    - [x] `rag_engine.py`: LangChain 기반의 Retrieval/Generation 체인 구축
    - [x] Ollama(Llama3)와 pgvector 연동 테스트
    - [x] 생성 실패 시를 대비한 Fallback 로직 적용
- [ ] **T1.4. 실시간 법령 수집기 (Law Scanner) 개발**
    - [ ] 국가법령정보센터 API 키 확보 및 연동 모듈 개발
    - [ ] 주요 부처 RSS 피드 크롤러 및 데이터 정규화 로직 구현
    - [ ] Redis 기반의 중복 수집 방지(Hash 비교) 시스템 구축
- [ ] **T1.5. 지능형 영향도 분석(Impact Analysis) 고도화**
    - [ ] 기존 커리큘럼-법령 간의 의존성 매핑 알고리즘 개발
    - [ ] 변경 수준(High/Medium/Low) 판별 프롬프트 최적화
