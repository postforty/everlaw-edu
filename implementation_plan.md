...
- `[ ]` **🔥 [U-6] 지능형 변형 퀴즈 즉석 출제 (Adaptive AI Quiz Clinic)**
...
- `[ ]` **AI DB 커넥션 최적화**: SQLAlchemy 커넥션 풀 소켓 누수 차단 및 싱글톤화.
- `[ ]` **비동기 마이그레이션**: LangGraph 워크플로우를 비동기(Async) 체계로 마이그레이션하여 FastAPI 추론 지연 및 블로킹 I/O 해결.
- `[ ]` **GitHub API 예외 처리**: 깃허브 Rate Limit 예외 처리 및 토큰 주입 정밀화.
